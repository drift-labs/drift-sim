...
def setup_ch(base_spread=0, strategies='', n_steps=100):
    ...
    # market three
    prices, timestamps = rand_heterosk_oracle(120, n_steps=n_steps)
    oracle = Oracle(prices=prices, timestamps=timestamps)
    amm = SimulationAMM(
        oracle=oracle, 
        base_asset_reserve=367_6 * AMM_RESERVE_PRECISION,
    ...
    market3 = SimulationMarket(amm=amm, market_index=2)

    markets = [market, market2, market3]
...
def main():
    path = Path('../../experiments/init/three_markets')
